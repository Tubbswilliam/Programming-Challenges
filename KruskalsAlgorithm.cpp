#include<bits\stdc++.h>
/**Steps*
->Sort all the edges in a non-decreasing order of weights.
->	1)Pick the smallest edge.
	2)Check if adding the edge forms a cycle.
	3)If a cycle is not formed-> Include edge ELSE exclude it.
->Repeat step 2 unless v-1 edges are included.
In the implementation of this algorithm we use the path compression and
union by rank techniques.
The time complexity is O(E log E + E log v)
